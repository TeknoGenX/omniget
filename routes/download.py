...
HEX_TASK_ID_REGEX = re.compile(r'^[a-f0-9]{32}$')
...
def start_download():
    ...
    non_media_exts = ('.iso', '.zip', '.rar', '.7z', '.tar', '.gz', '.tgz', '.pdf', '.exe', '.deb', '.dmg', '.apk', '.bin', '.img')
    if parsed_path.endswith(non_media_exts):
        with download_tasks_lock:
            download_tasks[task_id] = {
                'status': 'downloading',
                'progress': 0.0,
...
def get_download_status(task_id):
    if not HEX_TASK_ID_REGEX.match(task_id):
...
def get_download_file(task_id):
    ...
    if task_data['status'] != 'completed':
        return jsonify({'success': False, 'error': f"File not ready. Status: {task_data['status']}"}), 400
        
    filepath = task_data['filepath']
    download_name = clean_filename(task_data['filename'] or 'download')
    
    if not filepath or not os.path.exists(filepath):
        return jsonify({'success': False, 'error': 'File not found on server'}), 404
        
    # Enforce path safety confinement
...
def download_subtitle():
    data = request.json or {}
    url = data.get('url')
    lang = clean_filename(data.get('lang', 'en'))
    sub_format = clean_filename(data.get('format', 'srt'))
...
def download_lyrics(task_id):
    # Enforce strict 32-hex task_id validation to prevent path traversal
    if not HEX_TASK_ID_REGEX.match(task_id):
        return jsonify({'success': False, 'error': 'Invalid task ID'}), 400
    ...
    # Verify path confinement inside DOWNLOAD_DIR
    real_download_dir = os.path.realpath(DOWNLOAD_DIR)
    real_file_path = os.path.realpath(filepath)
    if not real_file_path.startswith(real_download_dir) or not os.path.exists(real_file_path):
        return jsonify({'success': False, 'error': 'Lyrics not found'}), 404
        
    return send_file(real_file_path, as_attachment=True, download_name=f"lyrics.{fmt}")
